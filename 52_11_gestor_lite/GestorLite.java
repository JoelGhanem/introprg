...
public class GestorLite {
	public static void main(String[] args) {
		...
		public static void mostraClient(Client client) {
			...
			for (int i = 1; i < client.getSize(); i++) {
				System.out.printf("%d. vehicle: %s%n        dies llogats: %d%n", i+1,client.getLloguers(i).getVehicle().getMarca(),client.getLloguers(i).getVehicle().getModel(),client.getLloguers(i).getDies());
			}
		}
}
