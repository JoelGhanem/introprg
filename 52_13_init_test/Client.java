...
System.out.print("Informe de lloguers del client ");
			for(int i = 0; i < lloguers.size(); i++) {
			System.out.printf("%s (%s)%n	%s %s: %s€",getNom(),getNif(),getLloguers(i).getVehicle().getMarca(),getLloguers(i).getVehicle().getModel(),"13");
			}
			return "";
    }
}
